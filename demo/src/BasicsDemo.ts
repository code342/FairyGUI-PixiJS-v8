...
export class BasicDemo {
    ...
    constructor() {
        ...
        fgui.UIPackage.loadPackage("resources/ui/Basics", this.onUILoaded.bind(this));
    }
    ...
    private runDemo(evt: PIXI.FederatedEvent): void {
    ...
    private onClickBack(evt: PIXI.FederatedEvent): void {
    ...
    private playText(): void {
        ...
        obj.getChild("n12").on("link", this.__clickLink, this);
    ...
    private __clickPopup1(evt: PIXI.FederatedEvent): void {
    ...
    private playDragDrop(): void {
        ...
        btnB.draggable = true;
        btnB.on(fgui.Events.DRAG_START, this.__onDragStart, this);

        var btnC: fgui.GButton = obj.getChild("c").asButton;
        btnC.icon = null;
        btnC.on(fgui.Events.DROP, this.__onDrop, this);

        var btnD: fgui.GObject = obj.getChild("d");
        btnD.draggable = true;
        var bounds: fgui.GObject = obj.getChild("bounds");
        var rect: PIXI.Rectangle = bounds.localToGlobalRect(0, 0, bounds.width, bounds.height);
    ...
    private __onDragStart(evt: PIXI.FederatedEvent): void {
    ...
    //------------------------------
    private startPos: PIXI.Point = new PIXI.Point();
    ...
    private playGrid(): void {
        ...
        for (var i: number = 0; i < cnt; i++) {
            ...
            item.getChild("t2").asTextField.color = fgui.ToolSet.toHexColor(testColors[Math.floor(Math.random() * 4)]);
    ...
    private playProgressBar(): void {
        ...
        fgui.Timer.shared.frameLoop(2, this, this.__playProgress);
        obj.on(fgui.Events.UNDISPLAY, this.__removeTimer, this);
    }

    private __removeTimer(): void {
        fgui.Timer.shared.clear(this, this.__playProgress);
    }
